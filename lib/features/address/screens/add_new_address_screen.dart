...
class _AddNewAddressScreenState extends State<AddNewAddressScreen> {
                            ...
                            builder: (context, lat, child) {
                              return Column(children: [
                                if (widget.address != null) ...[
                                  if (locationProvider.pickedAddressLongitude ==
                                          null &&
                                      locationProvider.pickedAddressLatitude ==
                                          null) ...[
                                    const WithoutMapWidget(),
                                  ],
                                  if (locationProvider.pickedAddressLatitude !=
                                          null &&
  ...
  void _initLoading() async {
    ...
    addressProvider.setCountryCode(CountryCode.fromCountryCode(
    ...
    if (widget.isUpdateEnable && widget.address != null) {
      ...
      _contactPersonNumberController
          .text = PhoneNumberCheckerHelper.getPhoneNumber(
              ...
              addressProvider.countryCode ?? '+91') ??
          '';
      ...
      addressProvider.setCountryCode(
          PhoneNumberCheckerHelper.getCountryCode(userModel?.phone ??
                  CountryCode.fromCountryCode(configModel.countryCode!)
                      .dialCode) ??
              '',
          isUpdate: true);
